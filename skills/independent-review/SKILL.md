...
3. **Fresh-eyes host-agent pass** — a read-only sub-agent (or the vendored
   ...
   the strict prompt below. Never reuse the authoring conversation.
...
**Independence rule:** skip the tier that matches the HOST agent's own model
family — under Claude Code the fresh-eyes Claude pass is the *same-family* seat
(still valuable, run it), and Codex+Gemini are the cross-model seats; under a
Codex install it is the reverse: Codex is same-family, so Claude/Gemini become
the required cross-model seats.

...
# same skill) and what does not (the main checkout, another skill, another repo) — plus
# the two ways the guard turns itself off, each of which must SAY so.
#
...
# Both sides must exist: two unresolvable paths would otherwise compare equal and turn a
# broken fixture into a passing assertion (round 2, both reviewers).
same_dir() {
  local a b
  [ -d "$1" ] && [ -d "$2" ] || return 1
  a="$(cd -P "$1" && pwd)"; b="$(cd -P "$2" && pwd)"
  [ "$a" = "$b" ]
}

# build_repo <dir> <installer> — a repo with four skills, alpha differing between two
# commits. The installer is committed BEFORE both, so a worktree at the first commit can
# run it (that is how the "installing from a linked worktree" cases work).
build_repo() {
...
one_installer() {
  local script="$1" var="$2" label="$3" R O DEST PIN PIN2 BROKEN out rc first
  R="$T/$label/repo"; O="$T/$label/other"; DEST="$T/$label/dest"
  PIN="$T/$label/pin"; PIN2="$T/$label/pin2"; BROKEN="$T/$label/broken"
  mkdir -p "$O/skills/beta" "$DEST" "$BROKEN/skills/epsilon"
  ...
  local DEST2="$T/$label/dest2"; mkdir -p "$DEST2"
...
# installer must relink everything rather than trust junk — and must SAY the guard is off.
...
old_git() {
  local script="$1" var="$2" variant="$3" R DEST BIN first rc probe
  ...
  check "old git ($variant, $script): says the guard is off" grep -q "pin detection is off" <<<"$out"
}

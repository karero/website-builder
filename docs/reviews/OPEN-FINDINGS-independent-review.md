...
Last updated 2026-09-11, after the mechanism-claim prompt change added B-REFUSAL-TEXT and
...
| B-REFUSAL-TEXT | `independent_review.sh` `looks_like_review()` | Refusal detection is a text test on responses with at most one finding, and text cannot separate a refusal from a finding. Two refusal-shaped findings ("1. BUG — I cannot review the file." / "2. RISK — I cannot access the repository.") are accepted as a review; a lone real finding saying "the handler cannot return JSON" is rejected; and a lone honest finding that cannot read its evidence is rejected even when marked UNVERIFIABLE, as `PROMPT_CORE` asks. Two exemptions for the last case (the marker; the marker plus a file:line anchor) were tried and withdrawn on 2026-09-11, because each let a refusal through. Needs a design change, such as a status the reviewer states apart from its findings, not a further regex. Pre-existing: all three reproduce on the origin/main function. `test_looks_like_review.sh` pins all three as KNOWN WRONG cases, so a fix has to change them on purpose. | Codex 2026-09-11 (rounds 4-6) |

## RISK — open

| id | Location | Finding | Found |
|---|---|---|---|
| R-SANDBOX | `independent_review.sh`, comments above `PROMPT_CORE` | The comments say `-s read-only` "already blocks writes" and that "the real boundary is the sandbox". Passing the flag shows the configuration requested, not what the CLI enforces, and no test of enforcement was checked. Pre-existing; raised by the new mechanism-claim sentence on its first run over this file. | Codex 2026-09-11 (round 5) |
